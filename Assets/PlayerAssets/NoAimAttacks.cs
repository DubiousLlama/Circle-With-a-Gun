...
public class NoAimAttacks : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (beamCooldown > beamRate && stationaryTime > 0.1f)
        {
            closest = closestFoe(foes);

            if (Vector3.Distance(transform.position, foes[closest].transform.position) < range)
            {
        ...
        void BeamAttack(GameObject foe)
        {
            ...
            lr.SetPosition(1, foe.transform.position);
